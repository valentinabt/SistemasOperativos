...
if options.jlist != '':
    ...
    for j in allJobs:
        ...
        job[jobCnt] = {'currPri':hiQueue, 'ticksLeft':quantum[hiQueue],
                       ...
                       'firstRun':-1, 'waitingTime':0, 'finished': False}
...
def isReady(j):
    return (j != currJob and job[j]['doingIO'] == False) and job[j]['finished'] == False
...
while finishedJobs < totalJobs:
    ...
    if currTime % 2 == 0:
         print('[ time %d ] THROUGHPUT %.2f' % (currTime, float(finishedJobs)/float(currTime)))
